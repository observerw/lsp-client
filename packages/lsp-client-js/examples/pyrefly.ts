import { LocalServer, LSPClient, SyncFeature, HoverFeature, DefinitionFeature, DiagnosticsFeature, toURI } from '../src';
...
async function testPyrefly() {
  ...
  for await (const lsp of client.start()) {
    ...
    await lsp.sync.didOpen(lsp, uri, 'python', `
...
bad_result = greet(123)
    ...
    try {
      ...
      console.log('Hover request failed:', e instanceof Error ? e.message : String(e));
    }
    ...
    try {
      ...
      console.log('Definition request failed:', e instanceof Error ? e.message : String(e));
    }
