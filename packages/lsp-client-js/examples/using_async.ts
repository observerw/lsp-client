...
async function processFile() {
	...
	client.diagnostics.onDiagnostics((params) => {
		console.log(
			`🔍 Diagnostics for ${params.uri.split("/").pop()}: ${params.diagnostics.length} issues`,
		);
	});
...
bad_count = process_data([1, 2, 3])  # Numbers instead of strings
	...
	console.log(
		"✅ Processing complete - all resources will be disposed automatically",
	);
}
