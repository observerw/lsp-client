import { LocalServer, LSPClient, SyncFeature, DefinitionFeature, toURI } from '../src';
...
class RustClient extends LSPClient {
  ...
  protected getLanguageId() { return 'rust'; }
...
async function run() {
  ...
  for await (const lsp of client.start()) {
    ...
    await lsp.sync.didOpen(lsp, uri, 'rust', 'fn main() { let x = 5; println!("{}", x); }');
