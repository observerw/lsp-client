...
class LSPClientBase(
    ...
    def __post_init__(self):
        self.logger = logging.getLogger(f"lsp_client.{self.__class__.__name__}")
    ...
    def auto_install(self, base_path: AnyPath | None = None) -> None:
        ...
        raise NotImplementedError(
            f"{self.__class__.__name__} does not support auto-installation of LSP server. Please install the server manually."
        )
    ...
    async def start(
        ...
        *,
        server_count: int = 1,
        server_info: LSPServerInfo = LSPServerInfo(),
